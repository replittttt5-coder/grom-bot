...
async def admin_flow_handler(event):
    ...
    if state["action"] == "create_cheque":
        ...
        me = await client.get_me()
        link = f"https://t.me/{me.username}?start=check_{cheque_id}"
        await event.reply(
            f"✅ Чек создан!\n\n"
            f"💰 Сумма: <b>{fmt(amount)} GROM</b>\n"
            f"🔗 Ссылка (одноразовая, кто первый откроет — тот заберёт):\n{link}",
...
HELP_TEXT = (
    ...
    "• <code>чек [сумма]</code> — создать чек-ссылку со своего баланса, скинь её кому угодно — GROM заберёт тот, кто первый откроет\n"
...
# ========== ЧЕК ПО ССЫЛКЕ (одноразовый, активируется переходом по ссылке) ==========
...
async def create_cheque_start(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        await event.answer("Доступ запрещён", alert=True)
        return
    admin_flow_state[event.sender_id] = {"action": "create_cheque", "step": "amount"}
    await event.respond("🔗 <b>Создание чека-ссылки</b>\n\nВведите сумму GROM для чека:", parse_mode="html")
    await event.answer()
...
async def list_cheques(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        await event.answer("Доступ запрещён", alert=True)
        return
    active = {cid: c for cid, c in data["cheques"].items() if not c["used"]}
    if not active:
        await event.respond("Активных чеков-ссылок нет.")
    else:
        me = await client.get_me()
        buttons = [
            [Button.inline(f"🗑 #{cid} — {fmt(c['amount'])} GROM", data=f"delcheque:{cid}")]
            for cid, c in active.items()
        ]
        lines = ["🔗 <b>Активные чеки-ссылки</b>\n"]
        for cid, c in active.items():
            lines.append(f"#{cid} — {fmt(c['amount'])} GROM\nhttps://t.me/{me.username}?start=check_{cid}")
...
async def redeem_cheque(user_id: int, cheque_id: str) -> str:
    """Активирует чек-ссылку. Возвращает текст результата для показа пользователю."""
    cheque = data["cheques"].get(cheque_id)
    if not cheque:
        return "❌ Чек не найден или уже был активирован."
    if cheque["used"]:
        return "❌ Этот чек уже активирован."
    cheque["used"] = True
    cheque["used_by"] = user_id
    add_balance(user_id, cheque["amount"])
    save_data(data)
    return f"✅ Чек активирован! Начислено: <b>{fmt(cheque['amount'])} GROM</b>\n💰 Новый баланс: {fmt(get_user_balance(user_id))} GROM"


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^чек\s+\d+$", norm(e.raw_text.lower().strip()))))
async def create_user_cheque(event):
    user_id = event.sender_id
    amount = int(norm(event.raw_text.strip()).split()[-1])
