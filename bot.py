...
DAILY_GIVE_LIMITS = {
    ...
    # зам и главный админ — без ограничений
}
...
COMPANY_WITHDRAW_MIN = 200000   # с какой суммы можно подать заявку на вывод
...
monitor_client: Optional[TelegramClient] = None  # аккаунт-наблюдатель за переводами @valyutaTG_bot
...
def apply_defaults(d: dict) -> dict:
    ...
    d.setdefault("checks", {})  # code_lower -> {amount, used, used_by, display_code, created_by} — промокоды
    d.setdefault("cheques", {})  # cheque_id -> {amount, used, used_by, created_by, at} — чеки по ссылке
    d.setdefault("fast_giveaways", {})  # fast_id -> {amount, total, claimed: [], active, created_by}
    d.setdefault("next_fast_id", 1)
    d.setdefault("log_cheques", [])    # список логов по чекам/промокодам (создание/активация)
    d.setdefault("log_transfers", [])  # список логов переводов между игроками (команда "п")
    # Донат за GRAM (внешняя валюта через @valyutaTG_bot)
    d.setdefault("monitor_session", None)   # строка сессии аккаунта-наблюдателя (StringSession)
    d.setdefault("monitor_phone", None)     # номер телефона подключённого аккаунта (для отображения)
    d.setdefault("gram_rate", 400)           # сколько GROM за 1 GRAM (покупка)
    d.setdefault("withdraw_gram_rate", 100)  # сколько GROM за 1 GRAM (вывод)
    d.setdefault("gram_recipient_id", 8791095325)  # кому в @valyutaTG_bot переводят GRAM
    d.setdefault("gram_orders", {})         # code -> {user_id, gram_amount, grom_amount, status, at}
    d.setdefault("gram_withdrawals", {})    # wid -> {user_id, grom_amount, gram_amount, status, at}
    d.setdefault("next_gram_withdrawal_id", 1)
...
GAME_KEYS = ["рулетка", "мины", "блэкджек", "краш", "башня", "монетка", "hilo", "квак", "футбол", "баскетбол"]
CHANCE_ADJUSTABLE_GAMES = [g for g in GAME_KEYS if g not in ("краш", "футбол", "баскетбол")]  # исход этих игр решает сам Telegram, шанс не настраивается
GAME_LABELS = {
    "рулетка": "🎰 Рулетка",
    "мины": "💣 Мины",
    "блэкджек": "🃏 Блэкджек",
...
def log_event(category: str, entry: dict):
    """Добавляет запись в лог (category: 'log_cheques' или 'log_transfers'), храня последние LOG_MAX_ENTRIES."""
    entry = dict(entry)
    entry["at"] = datetime.now().isoformat()
    data[category].append(entry)
    if len(data[category]) > LOG_MAX_ENTRIES:
        data[category] = data[category][-LOG_MAX_ENTRIES:]
    save_data(data)
...
def can_manage_admins(user_id: int) -> bool:
    """Только главный админ и «зам» могут назначать/снимать персонал."""
    r = admin_rank(user_id)
    return r >= 100 or r >= RANK_RIGHT_HAND


def can_ban_target(actor_id: int, target_id: int) -> bool:
    """Нельзя банить главного админа, если у тебя не ранг 'зам' (или ты сам главный админ)."""
...
# ========== ДОНАТ ЗА GRAM (мониторинг через аккаунт-наблюдатель) ==========
GRAM_BOT_USERNAME = "valyutaTG_bot"
GRAM_ORDER_TTL_MINUTES = 5  # сколько минут даётся на оплату, потом заявка сгорает
# Ищем в сообщении от @valyutaTG_bot сумму перевода и код в поле "💬 ..."
GRAM_NOTIFY_RE = re.compile(r"получил[аи]?\s+перевод\s+([\d\s]+)\s*GRAM.*?💬\s*(\S+)", re.S | re.IGNORECASE)


def generate_gram_code() -> str:
    while True:
        code = "".join(random.choices(string.ascii_uppercase + string.digits, k=7))
        if code not in data["gram_orders"]:
            return code


async def handle_gram_notification(event):
    """Разбирает входящее сообщение от @valyutaTG_bot и, если код совпал, зачисляет GROM."""
    text = event.raw_text or ""
    m = GRAM_NOTIFY_RE.search(text)
    if not m:
        return
    try:
    ...
    order = data["gram_orders"].get(code)
    if not order:
        await notify_main_admins(
            f"⚠️ Пришла оплата {fmt(amount)} GRAM с кодом <code>{code}</code>, "
            f"но заявка не найдена (истёк 5-минутный срок или код неверный). Проверьте вручную."
        )
        return
    if order["status"] != "pending":
        return

    if order["gram_amount"] != amount:
        await notify_main_admins(
            f"⚠️ Код <code>{code}</code> найден, но сумма не совпадает: "
            f"ожидалось {fmt(order['gram_amount'])} GRAM, пришло {fmt(amount)} GRAM. "
            f"GROM не начислен, проверьте вручную.",
        )
        return
    ...
    save_data(data)
    add_balance(order["user_id"], order["grom_amount"])
    log_event("log_cheques", {
        "type": "донат GRAM", "action": "оплачен", "code": code,
        "amount": order["grom_amount"], "by": order["user_id"],
    })

    try:
        await client.send_message(
            order["user_id"],
            f"✅ Оплата получена! Начислено: <b>{fmt(order['grom_amount'])} GROM</b>\n"
            f"💰 Новый баланс: {fmt(get_user_balance(order['user_id']))} GROM",
            parse_mode="html",
        )
    except Exception:
        pass
    await notify_main_admins(
        f"💎 Оплата GRAM подтверждена: {fmt(amount)} GRAM → {fmt(order['grom_amount'])} GROM "
        f"для пользователя <code>{order['user_id']}</code>."
    )


async def gram_orders_watcher():
    """Фоновая задача: удаляет просроченные (>5 минут) неоплаченные заявки на GRAM, чтобы не копились."""
    while True:
        try:
            now = datetime.now()
            expired = []
            for code, order in list(data["gram_orders"].items()):
                if order["status"] != "pending":
                    continue
                expires_raw = order.get("expires_at")
                if not expires_raw:
                    continue
                if now >= datetime.fromisoformat(expires_raw):
                    expired.append(code)
            if expired:
                for code in expired:
                    data["gram_orders"].pop(code, None)
                save_data(data)
        except Exception:
            pass
...
async def start_monitor_client():
    """Запускает (или перезапускает) аккаунт-наблюдатель, если сессия сохранена."""
    global monitor_client
    if not data.get("monitor_session"):
        return False
    if monitor_client and monitor_client.is_connected():
        return True
    try:
        monitor_client = TelegramClient(StringSession(data["monitor_session"]), API_ID, API_HASH)
        await monitor_client.connect()
        if not await monitor_client.is_user_authorized():
            return False

        @monitor_client.on(events.NewMessage(from_users=GRAM_BOT_USERNAME))
        async def _on_gram_msg(event):
            await handle_gram_notification(event)
...
def main_keyboard():
    return [
        ...
        [Button.text("🏢 Компания", resize=True), Button.text("🧾 Чек", resize=True)],
        [Button.text("💸 Вывод", resize=True)],
    ]
...
async def cmd_start(event):
    ...
    if deep_param == "bonus":
        ...
        if can:
            ...
            await event.respond(f"🎁 Вам начислено: 1 000 GROM\n💰 Новый баланс: {fmt(get_user_balance(user_id))}", buttons=main_keyboard())
        else:
            hours = remaining // 3600
            minutes = (remaining % 3600) // 60
            await event.respond(f"⏳ Осталось подождать {hours:02d}:{minutes:02d} до следующего бонуса", buttons=main_keyboard())
        return

    if deep_param.startswith("check_"):
        cheque_id = deep_param.split("_", 1)[1]
        result_text = await redeem_cheque(user_id, cheque_id)
        await event.respond(result_text, parse_mode="html", buttons=main_keyboard())
        return
...
async def cmd_adm(event):
    ...
    if rank >= RANK_SENIOR:
        keyboard += [
            ...
            [Button.inline("🔗 Создать чек-ссылку", data="create_cheque_start"), Button.inline("📋 Список чек-ссылок", data="list_cheques")],
            [Button.inline("📜 Логи чеков", data="log_cheques_view"), Button.inline("📜 Логи переводов", data="log_transfers_view")],
    ...
    if can_manage_admins(user_id):
        keyboard += [
            ...
            [Button.inline("🗄 ВОЗВРАТ ДАННЫХ", data="restore_data_start"), Button.inline("💾 Сохранить data.json", data="save_data_start")],
            [Button.inline("➕ Добавить аккаунт (GRAM)", data="add_account_start")],
        ]
    if user_id in ADMIN_IDS:
        keyboard += [
            [Button.inline("📨 Заявки на вывод", data="withdrawals_list")],
            [Button.inline("⚙️ Курс/получатель GRAM", data="gram_settings_start")],
        ]
...
ACTION_MIN_RANK = {
    ...
    "withdrawals_list": 100,  # заявки на вывод — только главный админ
...
# ========== АККАУНТ-НАБЛЮДАТЕЛЬ ЗА GRAM ==========
@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "add_account_start"))
async def add_account_start(event):
    if not can_manage_admins(event.sender_id):
        await event.answer("Доступ запрещён", alert=True)
        return
    admin_flow_state[event.sender_id] = {"action": "add_account", "step": "phone"}
    await event.respond(
        "➕ <b>Подключение аккаунта-наблюдателя</b>\n\n"
        "⚠️ Это реальный вход в личный Telegram-аккаунт (тот, что получает переводы GRAM от @valyutaTG_bot). "
        "Сессия сохранится в data.json — храните файл в надёжном месте, доступ к нему = полный доступ к аккаунту.\n\n"
...
@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "gram_settings_start"))
async def gram_settings_start(event):
    if event.sender_id not in ADMIN_IDS:
        await event.answer("Курс GRAM меняет только главный админ.", alert=True)
        return
    status = "🟢 подключён" if (monitor_client and monitor_client.is_connected()) else "🔴 не подключён"
    admin_flow_state[event.sender_id] = {"action": "gram_settings", "step": "rate"}
    await event.respond(
        f"⚙️ <b>Настройки GRAM</b>\n\n"
        f"Аккаунт-наблюдатель: {status}\n"
        f"Курс покупки: 1 GRAM = <b>{data['gram_rate']}</b> GROM\n"
        f"Курс вывода: 1 GRAM = <b>{data['withdraw_gram_rate']}</b> GROM\n"
        f"Получатель в @valyutaTG_bot: <code>{data['gram_recipient_id']}</code>\n\n"
        f"Введите новый курс <b>покупки</b> (сколько GROM за 1 GRAM):",
        parse_mode="html",
    )
    await event.answer()
...
@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "save_data_start"))
async def save_data_file(event):
    if not can_manage_admins(event.sender_id):
        await event.answer("Доступ запрещён", alert=True)
        return
    save_data(data)
    await event.answer()
    await event.respond(file=DATA_FILE, message="💾 Текущий data.json")
...
async def resolve_target(target: str):
    ...
    Для голого числового ID: если бот не может получить полную инфу о пользователе
    (частое ограничение ботов — нет доступа без предыдущего контакта), всё равно
    принимаем сам ID с заглушкой вместо имени, чтобы функции админки не ломались."""
    target = target.strip()
    if target.startswith("@"):
        try:
            entity = await client.get_entity(target)
            return entity.id, (getattr(entity, "first_name", None) or getattr(entity, "username", None) or str(entity.id))
        except Exception:
            return None, None
...
# ========== ОТМЕНА ОЖИДАНИЯ ВВОДА ЛЮБОЙ КНОПКОЙ ==========
# Если бот ждёт текстовый ввод (число шанса, название, эмодзи и т.д.), а человек вместо этого
# нажал любую ДРУГУЮ кнопку — ожидание сбрасывается. Кнопки, которые сами ЗАПУСКАЮТ такое
# ожидание (например «Создать чек» или «Забанить»), в исключениях — иначе они гасили бы
# собственное только что установленное состояние.
CONTINUATION_CALLBACK_EXACT = {
    "give_gram", "take_gram", "reset_user", "create_check", "company_price",
    "ban_user_start", "unban_user_start", "add_admin_start", "restore_data_start",
    "start_buy_company", "create_cheque_start", "add_account_start", "gram_settings_start",
    "pay_gram_start", "donate_custom_start", "withdraw_gram_start",
}
CONTINUATION_CALLBACK_PREFIXES = ("pick_rank:", "pick_emoji_slot:", "chance_pick:", "auc_pick:")


@client.on(events.CallbackQuery(func=lambda e: (
    e.data
    and e.data.decode() not in CONTINUATION_CALLBACK_EXACT
    and not e.data.decode().startswith(CONTINUATION_CALLBACK_PREFIXES)
)))
async def cancel_pending_input_on_button(event):
    uid = event.sender_id
    admin_flow_state.pop(uid, None)
    waiting_for_chance_game.pop(uid, None)
    waiting_for_company_name.pop(uid, None)
    waiting_for_emoji_slot.pop(uid, None)
    waiting_for_data_restore.pop(uid, None)
    waiting_for_user_cheque.pop(uid, None)
    waiting_for_gram_amount.pop(uid, None)
    waiting_for_custom_stars.pop(uid, None)
    waiting_for_gram_withdraw.pop(uid, None)


async def finish_account_login(event, temp_client, phone: str):
    session_string = temp_client.session.save()
    data["monitor_session"] = session_string
    data["monitor_phone"] = phone
    save_data(data)
    admin_flow_state.pop(event.sender_id, None)

    ok = await start_monitor_client()
    if ok:
        await event.reply("✅ Аккаунт подключён! Теперь слежу за переводами GRAM от @valyutaTG_bot.")
    else:
        await event.reply("⚠️ Аккаунт сохранён, но запустить наблюдение не удалось. Проверьте /adm ещё раз.")
...
async def admin_flow_handler(event):
    ...
    if state["action"] == "add_account":
        if state["step"] == "phone":
            ...
            try:
                ...
                await event.reply(f"❌ Не удалось отправить код: {ex}")
            ...
            state["step"] = "code"
            await event.reply("📩 Код отправлен. Введите код, который пришёл в Telegram на этот номер:")
            return
        ...
        if state["step"] == "code":
            ...
            code = text.strip()
            try:
                await temp_client.sign_in(state["phone"], code, phone_code_hash=state["phone_code_hash"])
            except SessionPasswordNeededError:
                state["step"] = "password"
                await event.reply("🔐 На аккаунте включена двухфакторная аутентификация. Введите пароль:")
                return
            except Exception as ex:
                await event.reply(f"❌ Неверный код: {ex}\nПопробуйте заново через /adm.")
                await temp_client.disconnect()
                admin_flow_state.pop(event.sender_id, None)
                return
            await finish_account_login(event, temp_client, state["phone"])
            return
        ...
        if state["step"] == "password":
            ...
            try:
                ...
                await event.reply(f"❌ Неверный пароль: {ex}\nПопробуйте ещё раз:")
                return
            await finish_account_login(event, temp_client, state["phone"])
            return

    if state["action"] == "gram_settings":
        if state["step"] == "rate":
            try:
                rate = int(text)
            except ValueError:
                await event.reply("❗ Введите курс целым числом (сколько GROM за 1 GRAM).")
                return
            if rate <= 0:
                await event.reply("❗ Курс должен быть больше 0.")
                return
            state["rate"] = rate
            state["step"] = "recipient"
            await event.reply(f"Введите ID получателя в @valyutaTG_bot (текущий: {data['gram_recipient_id']}):")
            return
        ...
        if state["step"] == "recipient":
            try:
                ...
                await event.reply("❗ Введите числовой ID.")
                return
            state["recipient"] = recipient_id
            state["step"] = "withdraw_rate"
            await event.reply(
                f"Введите курс <b>вывода</b> (сколько GROM за 1 GRAM, текущий: {data['withdraw_gram_rate']}):",
                parse_mode="html",
            )
            return

        if state["step"] == "withdraw_rate":
            try:
                withdraw_rate = int(text)
            except ValueError:
                await event.reply("❗ Введите курс целым числом.")
                return
            if withdraw_rate <= 0:
                await event.reply("❗ Курс должен быть больше 0.")
                return

            data["gram_rate"] = state["rate"]
            data["gram_recipient_id"] = state["recipient"]
            data["withdraw_gram_rate"] = withdraw_rate
            save_data(data)
            admin_flow_state.pop(event.sender_id, None)
            await event.reply(
                f"✅ Сохранено!\n"
                f"Курс покупки: 1 GRAM = {data['gram_rate']} GROM\n"
                f"Курс вывода: 1 GRAM = {data['withdraw_gram_rate']} GROM\n"
                f"Получатель: <code>{data['gram_recipient_id']}</code>",
                parse_mode="html",
            )
            return
    ...
    if state["action"] == "add_admin" and state["step"] == "id":
        ...
        await event.reply(
            ...
            buttons=[
                ...
                [Button.inline("4️⃣ Зам", data="pick_rank:4")],
            ],
            parse_mode="html",
        )
        return
    ...
    if state["action"] == "create_cheque_admin":
        if state["step"] == "amount":
            try:
                amount = int(text)
            except ValueError:
            ...
            state["step"] = "uses"
            await event.reply(
                "🔢 Сколько активаций у чека?\n"
                "• <code>1</code> — одноразовый\n"
                "• любое другое число — на несколько активаций (каждый получит полную сумму)",
                parse_mode="html",
            )
            return
        ...
        if state["step"] == "uses":
            try:
                ...
                await event.reply("❗ Введите число активаций целым числом.")
                return
            if uses <= 0:
                await event.reply("❗ Количество активаций должно быть больше 0.")
                return

            amount = state["amount"]
            cheque_id = generate_cheque_id()
            data["cheques"][cheque_id] = {
                "amount": amount,
            ...
            save_data(data)
            log_event("log_cheques", {
                "type": "чек-ссылка (админ)", "action": "создан", "code": cheque_id,
                "amount": amount, "uses": uses, "by": event.sender_id,
            })
            admin_flow_state.pop(event.sender_id, None)

            link = await build_cheque_link(cheque_id)
            await event.reply(
                f"✅ Чек создан!\n\n"
                f"💰 Сумма: <b>{fmt(amount)} GROM</b> × {uses} активаций\n"
                f"🔗 Ссылка:\n{link}",
                buttons=[[Button.url("💎 Открыть чек", link)]],
                parse_mode="html",
                link_preview=False,
            )
            return
    ...
    if state["action"] == "create_check":
        ...
        if state["step"] == "code":
            ...
            save_data(data)
            log_event("log_cheques", {
                "type": "промокод", "action": "создан", "code": code_display,
                "amount": amount, "uses": uses, "by": event.sender_id,
            })
            admin_flow_state.pop(event.sender_id, None)
            await event.reply(
...
async def transfer_gram(event):
    ...
    add_balance(target_id, amount)
    log_event("log_transfers", {
        "from_id": sender_id, "from_name": sender_name,
        "to_id": target_id, "to_name": target_name,
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^(бж|бл(э|е)к(джек)?)\s+\d+$", e.raw_text.lower().strip())))
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^(хило|хл|hl|hilo)\s+\d+$", norm(e.raw_text.lower().strip()))))
...
# ========== ИГРЫ: ФУТБОЛ / БАСКЕТБОЛ (настоящий бросок Telegram, подкрутить невозможно) ==========
# Официальная (пусть и незадокументированная) механика Telegram: у стикеров ⚽ и 🏀
# значение 1-5, где 4-5 — гол/попадание (40%), 1-3 — мимо (60%).
DICE_GOAL_MULT = 2.4   # реже выпадает — платит больше
DICE_MISS_MULT = 1.6   # чаще выпадает — платит меньше


async def play_dice_sport(event, game_key: str, emoji: str, choice: str, bet: int):
    user_id = event.sender_id

    dice_msg = await client.send_message(event.chat_id, file=InputMediaDice(emoticon=emoji))
    value = dice_msg.media.value
    actual_result = "гол" if value in (4, 5) else "мимо"
    await asyncio.sleep(4)  # даём анимации доиграть перед итогом

    won = (choice == actual_result)
    mult = DICE_GOAL_MULT if choice == "гол" else DICE_MISS_MULT

    if won:
        payout = int(bet * mult)
        add_balance(user_id, payout)
        apply_company_economy(game_key, True, bet, payout)
        await event.reply(
            f"{emoji} Выпало: <b>{value}/5 — {actual_result.upper()}</b>\n\n"
            f"✅ Угадал! Выигрыш: <b>{fmt(payout)} GROM</b> (x{mult})",
            parse_mode="html",
        )
    else:
        apply_company_economy(game_key, False, bet)
        await event.reply(
            f"{emoji} Выпало: <b>{value}/5 — {actual_result.upper()}</b>\n\n"
            f"❌ Не угадал. Проигрыш: <b>{fmt(bet)} GROM</b>",
            parse_mode="html",
        )


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^футбол\s+\d+\s+(гол|мимо)$", norm(e.raw_text.lower().strip()))))
async def play_football(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = int(parts[1])
    choice = parts[2]

    frozen, comp = is_game_frozen("футбол")
    if frozen:
        await event.reply(f"⛔ Футбол временно недоступен: компания-владелец «{comp['name']}» в долгу.")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    await play_dice_sport(event, "футбол", "⚽", choice, bet)


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^(баскетбол|баскет)\s+\d+\s+(гол|мимо)$", norm(e.raw_text.lower().strip()))))
async def play_basketball(event):
    user_id = event.sender_id
    parts = norm(event.raw_text.strip().lower()).split()
    bet = int(parts[1])
    choice = parts[2]

    frozen, comp = is_game_frozen("баскетбол")
    if frozen:
        await event.reply(f"⛔ Баскетбол временно недоступен: компания-владелец «{comp['name']}» в долгу.")
        return
    if bet <= 0:
        await event.reply("❗ Ставка должна быть больше 0.")
        return
    ...
    add_balance(user_id, -bet)
    await play_dice_sport(event, "баскетбол", "🏀", choice, bet)
...
HELP_TEXT = (
    ...
    "<b>⚽ Футбол / 🏀 Баскетбол</b>\n"
    "• <code>футбол [ставка] гол/мимо</code> — настоящий бросок Telegram, подкрутить нельзя\n"
    "• <code>баскетбол [ставка] гол/мимо</code> — то же самое, баскетбольным мячом\n\n"
    "<b>⚔️ Дуэли</b>\n"
    ...
    f"• <code>вывод компании [сумма]</code> — заявка на вывод от {fmt(COMPANY_WITHDRAW_MIN)} GROM (проверяет только главный админ)\n"
    "• <code>стата ком</code> — какая компания чем владеет (доступно в любом чате)\n"
    "• Игры на аукцион выставляет только администратор\n\n"
    "<b>🧾 Промокоды и чеки</b>\n"
    "• <code>активировать [код]</code> — активировать промокод и получить GROM (коды выдаёт администрация)\n"
    "• Кнопка «🧾 Чек» — создать чек-ссылку со своего баланса (сумма + число активаций), скинь её кому угодно\n"
    "• Чек по ссылке — просто перейди по присланной ссылке, GROM зачислится автоматически\n\n"
    "<b>📊 Прочее</b>\n"
...
async def donate_button(event):
    ...
    if monitor_client and monitor_client.is_connected():
        buttons.append([Button.inline("💎 Оплатить через GRAM", data="pay_gram_start")])
    buttons.append([Button.inline("💫 Пожертвовать", data="donate_custom_start")])
    await event.reply(
        "💎 <b>ДОНАТ</b> 💎\n"
        "━━━━━━━━━━━━━━━\n\n"
...
STAR_BASE_RATE = 250000  # GROM за 1 Star при свободном пожертвовании
waiting_for_custom_stars: dict = {}  # user_id -> True


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "donate_custom_start"))
async def donate_custom_start(event):
    waiting_for_custom_stars[event.sender_id] = True
    await event.respond(
        "💫 <b>Пожертвовать</b>\n\n"
        f"Введите сумму Stars, которую хотите отправить (от 1 ⭐, {fmt(STAR_BASE_RATE)} GROM за каждую):",
        parse_mode="html",
    )
    await event.answer()


@client.on(events.NewMessage(func=lambda e: e.raw_text and e.sender_id in waiting_for_custom_stars))
async def donate_custom_amount(event):
    user_id = event.sender_id
    text = event.raw_text.strip()
    ...
    try:
        ...
        return
    if stars < 1:
        await event.reply("❗ Минимум 1 Star.")
        return

    waiting_for_custom_stars.pop(user_id, None)
    grom = stars * STAR_BASE_RATE
    ok = await send_star_invoice(event.sender_id, stars, grom)
    if not ok:
        await event.reply(
        ...
        return
    if event.is_private:
        await event.reply(f"📩 Счёт на {stars} ⭐ ({fmt(grom)} GROM) отправлен выше.")
    else:
        await event.reply("📩 Счёт отправлен тебе в личные сообщения.")


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "pay_gram_start"))
async def pay_gram_start(event):
    if not (monitor_client and monitor_client.is_connected()):
        await event.answer("Оплата через GRAM сейчас недоступна.", alert=True)
        return
    waiting_for_gram_amount[event.sender_id] = True
    await event.respond(
        f"💎 <b>Оплата через GRAM</b>\n\n"
        f"Курс: 1 GRAM = {data['gram_rate']} GROM\n\n"
        f"Сколько GRAM хотите отправить?",
        parse_mode="html",
    )
    await event.answer()


waiting_for_gram_amount: dict = {}  # user_id -> True


@client.on(events.NewMessage(func=lambda e: e.raw_text and e.sender_id in waiting_for_gram_amount))
async def gram_amount_input(event):
    user_id = event.sender_id
    text = event.raw_text.strip()

    if text.startswith("/"):
        waiting_for_gram_amount.pop(user_id, None)
        return

    try:
        gram_amount = int(text)
    except ValueError:
    ...
    waiting_for_gram_amount.pop(user_id, None)
    grom_amount = gram_amount * data["gram_rate"]
    code = generate_gram_code()
    data["gram_orders"][code] = {
        "user_id": user_id,
        "gram_amount": gram_amount,
        "grom_amount": grom_amount,
        "status": "pending",
        "at": datetime.now().isoformat(),
        "expires_at": (datetime.now() + timedelta(minutes=GRAM_ORDER_TTL_MINUTES)).isoformat(),
    }
    save_data(data)

    recipient = data["gram_recipient_id"]
    pay_command = f"п {recipient} {gram_amount} {code}"
    await event.reply(
        f"💎 За <b>{fmt(gram_amount)} GRAM</b> начислим <b>{fmt(grom_amount)} GROM</b>.\n\n"
        f"Чтобы оплатить, отправьте это сообщение боту @{GRAM_BOT_USERNAME} "
        f"(скопируйте точно, как есть):\n\n"
        f"<code>{pay_command}</code>\n\n"
        f"⏳ На оплату даётся <b>{GRAM_ORDER_TTL_MINUTES} минут</b>, потом заявка сгорит.\n"
        f"После оплаты GROM начислится автоматически, обычно в течение минуты.",
        parse_mode="html",
    )


# ========== ВЫВОД GROM (в GRAM) ==========
@client.on(events.NewMessage(func=lambda e: e.raw_text == "💸 Вывод"))
async def withdraw_button(event):
    await event.reply(
        "💸 <b>Вывод</b>\n\nКуда хотите вывести?",
        buttons=[[Button.inline("💎 GRAM", data="withdraw_gram_start")]],
        parse_mode="html",
    )


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "withdraw_gram_start"))
async def withdraw_gram_start(event):
    waiting_for_gram_withdraw[event.sender_id] = True
    await event.respond(
        f"💎 <b>Вывод в GRAM</b>\n\n"
        f"Курс: {fmt(data['withdraw_gram_rate'] * 1000)} GROM = 1000 GRAM (то есть {data['withdraw_gram_rate']} GROM = 1 GRAM)\n"
        f"💰 Твой баланс: {fmt(get_user_balance(event.sender_id))} GROM\n\n"
        f"Сколько GROM хотите вывести?",
        parse_mode="html",
    )
    await event.answer()


waiting_for_gram_withdraw: dict = {}  # user_id -> True


@client.on(events.NewMessage(func=lambda e: e.raw_text and e.sender_id in waiting_for_gram_withdraw))
async def gram_withdraw_amount(event):
    user_id = event.sender_id
    text = event.raw_text.strip()

    if text.startswith("/"):
        waiting_for_gram_withdraw.pop(user_id, None)
        return

    try:
        grom_requested = int(text)
    except ValueError:
        await event.reply("❗ Введите число.")
        return
    if grom_requested <= 0:
    ...
    gram_amount = grom_requested // data["withdraw_gram_rate"]
    if gram_amount < 1:
        await event.reply(f"❗ Слишком маленькая сумма. Минимум {fmt(data['withdraw_gram_rate'])} GROM (= 1 GRAM).")
        return

    grom_used = gram_amount * data["withdraw_gram_rate"]
    if get_user_balance(user_id) < grom_used:
        await event.reply(f"❌ Недостаточно GROM. Нужно {fmt(grom_used)} GROM, на балансе {fmt(get_user_balance(user_id))} GROM.")
        return

    waiting_for_gram_withdraw.pop(user_id, None)
    add_balance(user_id, -grom_used)

    wid = str(data["next_gram_withdrawal_id"])
    data["next_gram_withdrawal_id"] += 1
    data["gram_withdrawals"][wid] = {
        "user_id": user_id,
        "grom_amount": grom_used,
        "gram_amount": gram_amount,
        "status": "pending",
        "at": datetime.now().isoformat(),
    }
    ...
    await event.reply(
        ...
        f"💰 Списано: <b>{fmt(grom_used)} GROM</b>\n"
        f"💎 К получению: <b>{fmt(gram_amount)} GRAM</b>\n\n"
        f"Ожидай подтверждения от администрации.",
        parse_mode="html",
    )
    ...
    for admin_id in ADMIN_IDS:
        try:
            await client.send_message(
                admin_id,
                f"💸 <b>Новая заявка на вывод в GRAM</b>\n"
                f"Игрок: {mention(user_id, buyer_name)}\n"
                f"Списано: {fmt(grom_used)} GROM\n"
                f"К выплате: {fmt(gram_amount)} GRAM\n"
                f"Заявка: #{wid}",
                buttons=[[
                    Button.inline("✅ Заявка выведена", data=f"gwd_ok:{wid}"),
                    Button.inline("❌ Отклонить", data=f"gwd_no:{wid}"),
                ]],
                parse_mode="html",
            )
        except Exception:
...
@client.on(events.CallbackQuery(func=lambda e: e.data and (e.data.decode().startswith("gwd_ok:") or e.data.decode().startswith("gwd_no:"))))
async def handle_gram_withdrawal_decision(event):
    if event.sender_id not in ADMIN_IDS:
        await event.answer("Заявки на вывод проверяет только главный админ.", alert=True)
        return

    action, wid = event.data.decode().split(":", 1)
    w = data["gram_withdrawals"].get(wid)
    ...
    if action == "gwd_ok":
        w["status"] = "done"
        save_data(data)
        await event.edit(f"✅ Заявка #{wid} выведена: {fmt(w['gram_amount'])} GRAM.")
        try:
            await client.send_message(
                w["user_id"],
                f"✅ Твоя заявка на вывод <b>{fmt(w['gram_amount'])} GRAM</b> выполнена администрацией.",
                parse_mode="html",
            )
        except Exception:
            pass
        ...
        w["status"] = "rejected"
        save_data(data)
        add_balance(w["user_id"], w["grom_amount"])
        await event.edit(f"❌ Заявка #{wid} отклонена, {fmt(w['grom_amount'])} GROM возвращены игроку.")
        try:
            await client.send_message(
                w["user_id"],
                f"❌ Твоя заявка на вывод {fmt(w['gram_amount'])} GRAM отклонена. "
                f"{fmt(w['grom_amount'])} GROM возвращены на баланс.",
                parse_mode="html",
            )
        except Exception:
            pass
...
async def send_star_invoice(user_id: int, stars: int, grom: int) -> bool:
    """Отправляет пользователю в личку счёт на оплату Stars за GROM. Возвращает True при успехе."""
    payload = f"grom_pack:{stars}:{grom}:{user_id}".encode()
    invoice = Invoice(
        currency="XTR",
    ...
    media = InputMediaInvoice(
        ...
        description=f"{fmt(grom)} GROM для бота GROM за {stars} Telegram Stars.",
        invoice=invoice,
        payload=payload,
        provider="",
...
async def stars_precheckout(update):
    try:
        ...
        print("⚠️ Ошибка подтверждения оплаты (precheckout):", ex)
        await notify_main_admins(
            f"⚠️ Не удалось подтвердить оплату Stars (precheckout, query_id={update.query_id}): {ex}\n"
            f"Если у покупателя списались Stars, но GROM не пришёл — начисли вручную через /adm."
        )
...
async def games_button(event):
    await event.reply(
        ...
        "⚽ <b>Футбол</b> / 🏀 <b>Баскетбол</b>\n"
        "Настоящий бросок Telegram — подкрутить невозможно! Угадай гол или мимо.\n"
        "Команда: <code>футбол 500 гол</code> или <code>баскетбол 500 мимо</code>\n\n"
        "⚔️ <b>Дуэль</b>\n"
...
# ========== ЧЕКИ (ваучеры) ==========
@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.startswith("/createpromo")))
async def create_promo_cmd(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        return
    ...
    if len(parts) < 4:
        await event.reply(
            "Формат: <code>/createpromo [сумма] [активаций] [название]</code>\n"
            "Пример: <code>/createpromo 33333 50 Обновление</code>",
            parse_mode="html",
        )
        return
    ...
    try:
        ...
        await event.reply("❗ Сумма и количество активаций должны быть числами.")
        return
    if amount <= 0 or uses <= 0:
        await event.reply("❗ Сумма и активации должны быть больше 0.")
        return

    code_display = parts[3].strip()
    if " " in code_display or not code_display:
        await event.reply("❗ Название промокода должно быть одним словом, без пробелов.")
        return

    code_key = norm(code_display.lower())
    if code_key in data["checks"]:
        await event.reply("❗ Промокод с таким названием уже существует.")
        return
...
# ========== ФАСТ (быстрая раздача в чате кнопкой) ==========
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^фаст\s+\d+\s+\d+$", norm(e.raw_text.lower().strip()))))
async def start_fast(event):
    if admin_rank(event.sender_id) < RANK_RIGHT_HAND:
        return

    parts = norm(event.raw_text.strip()).split()
    amount = int(parts[1])
    total = int(parts[2])
    if amount <= 0 or total <= 0:
        await event.reply("❗ Сумма и количество активаций должны быть больше 0.")
        return

    fast_id = str(data["next_fast_id"])
    data["next_fast_id"] += 1
    data["fast_giveaways"][fast_id] = {
        "amount": amount,
        "total": total,
        "claimed": [],
        "active": True,
    ...
    await event.reply(
        f"🎁 <b>ФАСТ!</b>\n\n"
        f"💰 {fmt(amount)} GROM каждому\n"
        f"👥 Осталось мест: <b>{total}/{total}</b>\n\n"
        f"Успей нажать кнопку ниже!",
        buttons=[[Button.inline("🎁 Получить", data=f"fast_claim:{fast_id}")]],
        parse_mode="html",
    )


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode().startswith("fast_claim:")))
async def fast_claim(event):
    fast_id = event.data.decode().split(":", 1)[1]
    giveaway = data["fast_giveaways"].get(fast_id)
    user_id = event.sender_id

    if not giveaway or not giveaway["active"]:
        await event.answer("❌ Фаст уже завершён.", alert=True)
        return
    if user_id in giveaway["claimed"]:
        await event.answer("❌ Ты уже получил из этого фаста!", alert=True)
        return
    if len(giveaway["claimed"]) >= giveaway["total"]:
        giveaway["active"] = False
        save_data(data)
        await event.answer("❌ Все места уже разобрали!", alert=True)
        return

    giveaway["claimed"].append(user_id)
    add_balance(user_id, giveaway["amount"])
    left = giveaway["total"] - len(giveaway["claimed"])
    if left <= 0:
        giveaway["active"] = False
    save_data(data)

    await event.answer(f"🎁 Вам начислено {fmt(giveaway['amount'])} GROM!", alert=True)

    try:
        if giveaway["active"]:
            await event.edit(
                f"🎁 <b>ФАСТ!</b>\n\n"
                f"💰 {fmt(giveaway['amount'])} GROM каждому\n"
                f"👥 Осталось мест: <b>{left}/{giveaway['total']}</b>\n\n"
                f"Успей нажать кнопку ниже!",
                buttons=[[Button.inline("🎁 Получить", data=f"fast_claim:{fast_id}")]],
                parse_mode="html",
            )
            ...
            await event.edit(
                f"🎁 <b>ФАСТ ЗАВЕРШЁН!</b>\n\n"
                f"💰 {fmt(giveaway['amount'])} GROM каждому\n"
                f"👥 Разобрано: <b>{giveaway['total']}/{giveaway['total']}</b>",
                buttons=None,
                parse_mode="html",
            )
    except Exception:
...
# ========== ЧЕК ПО ССЫЛКЕ (активируется переходом по ссылке, можно на несколько активаций) ==========
def generate_cheque_id() -> str:
    while True:
        cid = str(random.randint(100_000_000, 999_999_999))
        if cid not in data["cheques"]:
            return cid


async def build_cheque_link(cheque_id: str) -> str:
    me = await client.get_me()
    return f"https://t.me/{me.username}?start=check_{cheque_id}"


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "create_cheque_start"))
async def create_cheque_start(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        await event.answer("Доступ запрещён", alert=True)
        return
    admin_flow_state[event.sender_id] = {"action": "create_cheque_admin", "step": "amount"}
    await event.respond("🔗 <b>Создание чека-ссылки (админ, бесплатно)</b>\n\nВведите сумму GROM на одну активацию:", parse_mode="html")
    await event.answer()


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "list_cheques"))
async def list_cheques(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        await event.answer("Доступ запрещён", alert=True)
        return
    active = {cid: c for cid, c in data["cheques"].items() if c["activations_left"] > 0}
    if not active:
        await event.respond("Активных чеков-ссылок нет.")
    else:
        buttons = [
            [Button.inline(f"🗑 #{cid} — {fmt(c['amount'])} GROM ({c['activations_left']}/{c['activations_total']})", data=f"delcheque:{cid}")]
            for cid, c in active.items()
        ]
        lines = ["🔗 <b>Активные чеки-ссылки</b>\n"]
        for cid, c in active.items():
            link = await build_cheque_link(cid)
            lines.append(f"#{cid} — {fmt(c['amount'])} GROM x{c['activations_left']}/{c['activations_total']}\n{link}")
        await event.respond("\n\n".join(lines), buttons=buttons, parse_mode="html", link_preview=False)
    await event.answer()


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode().startswith("delcheque:")))
async def delete_cheque(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        await event.answer("Доступ запрещён", alert=True)
        return
    cid = event.data.decode().split(":", 1)[1]
    cheque = data["cheques"].pop(cid, None)
    save_data(data)
    if not cheque:
        await event.answer("Чек уже удалён.", alert=True)
        return
    await event.edit(f"🗑 Чек #{cid} на {fmt(cheque['amount'])} GROM удалён.")
    await event.answer()


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "log_cheques_view"))
async def log_cheques_view(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        await event.answer("Доступ запрещён", alert=True)
        return
    entries = data["log_cheques"][-30:][::-1]
    if not entries:
        await event.respond("Логов пока нет.")
    else:
        lines = ["📜 <b>Последние операции с чеками/промокодами</b>\n"]
        for e in entries:
            actor_name = await get_display_name(e["by"])
            action_word = "🟢 создан" if e["action"] == "создан" else "🔵 активирован"
            uses_part = f" x{e['uses']}" if "uses" in e else ""
            lines.append(
                f"{action_word} — {e['type']} <code>{e['code']}</code> — {fmt(e['amount'])} GROM{uses_part}\n"
                f"👤 {mention(e['by'], actor_name)} | {e['at'][:16].replace('T', ' ')}"
            )
        await send_chunks(event.chat_id, "\n\n".join(lines))
    await event.answer()


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "log_transfers_view"))
async def log_transfers_view(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        await event.answer("Доступ запрещён", alert=True)
        return
    entries = data["log_transfers"][-30:][::-1]
    if not entries:
        await event.respond("Логов пока нет.")
    else:
        lines = ["📜 <b>Последние переводы между игроками</b>\n"]
        for e in entries:
            comment_part = f"\n💬 {e['comment']}" if e.get("comment") else ""
            lines.append(
                f"{mention(e['from_id'], e['from_name'])} → {mention(e['to_id'], e['to_name'])}\n"
                f"💰 {fmt(e['amount'])} GROM | {e['at'][:16].replace('T', ' ')}{comment_part}"
            )
        await send_chunks(event.chat_id, "\n\n".join(lines))
    await event.answer()


async def redeem_cheque(user_id: int, cheque_id: str) -> str:
    """Активирует чек-ссылку (поддерживает несколько активаций). Возвращает текст результата."""
    cheque = data["cheques"].get(cheque_id)
    if not cheque:
        return "❌ Чек не найден."
    if user_id in cheque["used_by"]:
        return "❌ Ты уже активировал этот чек."
    if cheque["activations_left"] <= 0:
        return "❌ Этот чек уже полностью разобрали."

    cheque["activations_left"] -= 1
    cheque["used_by"].append(user_id)
    add_balance(user_id, cheque["amount"])
    save_data(data)
    log_event("log_cheques", {
        "type": "чек-ссылка", "action": "активирован", "code": cheque_id,
        "amount": cheque["amount"], "by": user_id,
    })
    return (
        f"✅ Чек активирован! Начислено: <b>{fmt(cheque['amount'])} GROM</b>\n"
        f"💰 Новый баланс: {fmt(get_user_balance(user_id))} GROM\n"
        f"🔢 Осталось активаций: {cheque['activations_left']}/{cheque['activations_total']}"
    )


waiting_for_user_cheque: dict = {}  # user_id -> {"step": "amount"/"uses", "amount": int}


@client.on(events.NewMessage(func=lambda e: e.raw_text == "🧾 Чек"))
async def cheque_button(event):
    user_id = event.sender_id
    waiting_for_user_cheque[user_id] = {"step": "amount"}
    await event.reply(
        "🧾 <b>Создание чека</b>\n\n"
        "Чек — это ссылка: кто по ней перейдёт, тот и заберёт GROM. Можно скинуть другу или в чат.\n\n"
        "Введите сумму GROM <b>на одну активацию</b>:",
        parse_mode="html",
    )


@client.on(events.NewMessage(func=lambda e: e.raw_text and e.sender_id in waiting_for_user_cheque))
async def cheque_flow_input(event):
    user_id = event.sender_id
    state = waiting_for_user_cheque[user_id]
    text = event.raw_text.strip()

    if text.startswith("/"):
        waiting_for_user_cheque.pop(user_id, None)
        return
    ...
    if state["step"] == "amount":
        ...
        await event.reply(
            "🔢 Сколько активаций у чека?\n"
            "• <code>1</code> — одноразовый (только для одного человека)\n"
            "• любое другое число — чек смогут забрать несколько разных людей, "
            "каждый получит полную сумму (спишется сумма × активации)",
            parse_mode="html",
        )
        return
    ...
    if state["step"] == "uses":
        ...
        if get_user_balance(user_id) < total_cost:
            await event.reply(
                f"❌ Недостаточно GROM. Нужно {fmt(total_cost)} GROM "
                f"({fmt(amount)} × {uses} активаций), на балансе {fmt(get_user_balance(user_id))} GROM."
            )
            waiting_for_user_cheque.pop(user_id, None)
            return
        ...
        save_data(data)
        log_event("log_cheques", {
            "type": "чек-ссылка", "action": "создан", "code": cheque_id,
            "amount": amount, "uses": uses, "by": user_id,
        })

        link = await build_cheque_link(cheque_id)
        await event.reply(
            f"✅ Чек создан!\n\n"
            f"💰 Сумма: <b>{fmt(amount)} GROM</b> × {uses} активаций (списано {fmt(total_cost)} GROM)\n"
            f"🔗 Ссылка:\n{link}\n\n"
            f"💡 Можешь скинуть эту ссылку кому угодно — другу, в чат, куда хочешь.",
            buttons=[[Button.url("💎 Открыть чек", link)]],
            parse_mode="html",
            link_preview=False,
        )
...
async def activate_check(event):
    ...
    save_data(data)
    log_event("log_cheques", {
        "type": "промокод", "action": "активирован", "code": check.get("display_code", code_key),
        "amount": check["amount"], "by": user_id,
    })
...
async def main():
    ...
    asyncio.create_task(gram_orders_watcher())
    if data.get("monitor_session"):
        ok = await start_monitor_client()
        print("Аккаунт-наблюдатель GRAM:", "подключён" if ok else "не удалось подключить")
