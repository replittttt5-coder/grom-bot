...
STAR_PACK_STARS = 10       # цена пакета в Telegram Stars
STAR_PACK_GROM = 50000     # сколько GROM выдаётся за пакет
...
GAME_KEYS = ["рулетка", "мины", "блэкджек", "краш", "башня", "монетка"]
...
waiting_for_company_chance: dict = {}  # user_id -> game_key, ожидаем ввод % шанса от владельца компании
...
def main_keyboard():
    return [
        ...
        [Button.text("Изменить язык", resize=True)],
    ]
...
async def cmd_adm(event):
    ...
    if rank >= RANK_SENIOR:
        keyboard += [
            ...
            [Button.inline("🏆 Создать турнир", data="create_tournament")],
            [Button.inline("🔚 Завершить турнир", data="end_tournament")],
...
ACTION_MIN_RANK = {
    ...
    "create_tournament": RANK_SENIOR,
    "end_tournament": RANK_SENIOR,
...
@client.on(events.CallbackQuery(
    ...
    ["upload_gif", "give_gram", "take_gram", "create_tournament", "end_tournament",
...
async def admin_callbacks(event):
    ...
    if action == "upload_gif":
        ...
        all_active = {tid: t for tid, t in tours.items() if t["status"] in ("open", "active")}
        if not all_active:
            await event.respond("Нет активных турниров.")
        else:
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^краш\s+\d+$", e.raw_text.lower().strip())))
async def start_crash(event):
    ...
    msg = await event.reply(crash_text(bet, 1.00), buttons=crash_keyboard(user_id), parse_mode="html")
    game["message"] = msg
...
# ========== ИГРА: БАШНЯ ==========
TOWER_LEVELS = 8
TOWER_TILES = 3  # 1 ловушка, 2 безопасные двери на этаже
...
HELP_TEXT = (
    ...
    "<b>💣 Мины</b>\n"
    "• <code>мины [ставка] [кол-во мин]</code> — начать игру (мин от 1 до 24)\n\n"
    "<b>🏗 Башня</b>\n"
    "• <code>башня [ставка]</code> — поднимайся по этажам, на каждом одна из 3 дверей — ловушка\n\n"
    "<b>🪙 Монетка</b>\n"
    ...
    "<b>🚀 Краш</b>\n"
    "• <code>краш [ставка]</code> — множитель растёт, забери выигрыш до краха\n\n"
    "<b>⚔️ Дуэли</b>\n"
    ...
    "• <code>очки</code> — твои очки в активных турнирах\n\n"
    "<b>🏰 Кланы</b>\n"
...
# ========== ТУРНИРЫ ==========
@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.startswith("/турнир")))
async def create_tournament(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        return
    ...
    try:
        ...
        await event.reply("❗ Взнос и кол-во мест должны быть числами.")
        return
    if entry_fee < 0 or max_slots < 2:
        await event.reply("❗ Взнос ≥ 0, мест минимум 2.")
        return
    ...
    fee_text = f"Взнос: <b>{fmt(entry_fee)} GROM</b>" if entry_fee > 0 else "Бесплатный вход"
    await event.reply(
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.startswith("/старт")))
async def start_tournament(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        return
    parts = event.raw_text.split()
    if len(parts) < 2:
        await event.reply("❗ Формат: <code>/старт [id]</code>", parse_mode="html")
        return
    tid = parts[1]
    tour = data["tournaments"].get(tid)
    if not tour:
        await event.reply(f"❌ Турнир #{tid} не найден.")
        return
    if tour["status"] != "open":
        await event.reply(f"❌ Турнир #{tid} уже {'идёт' if tour['status'] == 'active' else 'завершён'}.")
        return
    if len(tour["participants"]) < 2:
        await event.reply("❌ Нужно минимум 2 участника для старта.")
        return
    ...
    await event.reply(
        ...
        f"👥 Участников: {p_count} | 💰 Призовой фонд: {fmt(tour['prize_pool'])} GROM\n\n"
...
@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.startswith("/завершить")))
async def end_tournament(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        return
    parts = event.raw_text.split()
    if len(parts) < 2:
        await event.reply("❗ Формат: <code>/завершить [id]</code>", parse_mode="html")
        return
    tid = parts[1]
    tour = data["tournaments"].get(tid)
    if not tour:
    ...
    if tour["status"] == "finished":
        await event.reply(f"❌ Турнир #{tid} уже завершён.")
        return
    if tour["status"] == "open":
        await event.reply(f"❌ Турнир #{tid} ещё не стартовал. Сначала: <code>/старт {tid}</code>", parse_mode="html")
        return

    scores = tour.get("scores", {})
    if not scores:
        await event.reply("❌ Никто не набрал очков. Завершение невозможно.")
        return
    ...
    w_name = await get_display_name(winner_id)

    sorted_scores = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:5]
    medals = ["🥇", "🥈", "🥉", "4.", "5."]
    board_lines = []
    for i, (uid, sc) in enumerate(sorted_scores):
        uname = await get_display_name(int(uid))
    ...
    await event.reply(
        ...
        f"💰 Призовой фонд: <b>{fmt(prize)} GROM</b>\n\n"
        f"<b>Таблица лидеров:</b>\n" + "\n".join(board_lines) + f"\n\n"
        f"🥇 Победитель: {mention(winner_id, w_name)} ({fmt(winner_score)} очков) → +{fmt(prize)} GROM!",
        parse_mode="html",
    )


@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.startswith("/удалить")))
async def delete_tournament(event):
    if admin_rank(event.sender_id) < RANK_SENIOR:
        return
    parts = event.raw_text.split()
    if len(parts) < 2:
        await event.reply("❗ Формат: <code>/удалить [id]</code>", parse_mode="html")
        return
    tid = parts[1]
    tour = data["tournaments"].get(tid)
    if not tour:
    ...
    if tour["status"] == "finished":
        ...
        save_data(data)
        await event.reply(f"🗑 Турнир #{tid} удалён из истории.")
        return
    ...
    await event.reply(f"🗑 Турнир <b>#{tid} — {tour['name']}</b> удалён.{refund_text}", parse_mode="html")


@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.lower() == "турниры"))
async def list_tournaments(event):
    tours = data.get("tournaments", {})
    active = {tid: t for tid, t in tours.items() if t["status"] in ("open", "active")}
    if not active:
        await event.reply("🏆 Сейчас нет активных турниров.")
        return

    lines = ["🏆 <b>Активные турниры</b>\n"]
    for tid, t in active.items():
        fee_text = f"{t['entry_fee']} GROM" if t["entry_fee"] > 0 else "бесплатно"
        slots_left = t["max_slots"] - len(t["participants"])
        status_icon = "🟢 Набор" if t["status"] == "open" else "🔴 Идёт"
        lines.append(
            f"<b>#{tid} — {t['name']}</b> {status_icon}\n"
            f"   Взнос: {fee_text} | Мест осталось: {slots_left} | Приз: {fmt(t['prize_pool'])} GROM\n"
            + (f"   Вступить: <code>уч {tid}</code>" if t["status"] == "open" else "   🎰 Крутите рулетку!")
        )
    await event.reply("\n\n".join(lines), parse_mode="html")


@client.on(events.NewMessage(func=lambda e: e.raw_text and re.match(r"^уч(\s+\d+)?$", e.raw_text.lower().strip())))
async def join_tournament(event):
    user_id = event.sender_id
    sender = await event.get_sender()
    name = sender.first_name if sender else str(user_id)
    parts = event.raw_text.strip().split()
    ...
    if len(parts) < 2:
        tours = {tid: t for tid, t in data.get("tournaments", {}).items() if t["status"] == "open"}
        if len(tours) == 1:
            tid = list(tours.keys())[0]
        elif not tours:
            await event.reply("🏆 Нет турниров, принимающих участников.")
            return
        else:
            lines = ["Укажи номер турнира: <code>уч [id]</code>\n"]
            for tid, t in tours.items():
                fee_text = f"{t['entry_fee']} GROM" if t["entry_fee"] > 0 else "бесплатно"
                lines.append(f"• #{tid} — {t['name']} ({fee_text}, {len(t['participants'])}/{t['max_slots']} мест)")
            await event.reply("\n".join(lines), parse_mode="html")
            return
    else:
        tid = parts[1]
    ...
    if not tour:
        ...
        return
    if tour["status"] != "open":
        await event.reply(f"❌ Набор в турнир #{tid} закрыт.")
        return
    if user_id in tour["participants"]:
        await event.reply(f"✅ Ты уже записан в <b>{tour['name']}</b>!", parse_mode="html")
        return
    if len(tour["participants"]) >= tour["max_slots"]:
        await event.reply(f"❌ В турнире <b>{tour['name']}</b> все места заняты!", parse_mode="html")
        return
    ...
    fee_text = f" (списано {fmt(fee)} GROM)" if fee > 0 else ""
    await event.reply(
        f"✅ {mention(user_id, name)} записан в турнир <b>{tour['name']}</b>!{fee_text}\n"
        f"👥 {len(tour['participants'])}/{tour['max_slots']} мест | 💰 Призовой фонд: {fmt(tour['prize_pool'])} GROM\n"
        + (f"⏳ Осталось мест: {slots_left}" if slots_left > 0 else "✅ Все места заняты!"),
        parse_mode="html",
    )


@client.on(events.NewMessage(func=lambda e: e.raw_text and e.raw_text.lower() == "очки"))
async def my_tournament_score(event):
    user_id = event.sender_id
    sender = await event.get_sender()
    name = sender.first_name if sender else str(user_id)
    active_tours = {
        tid: t for tid, t in data.get("tournaments", {}).items()
        if t["status"] == "active" and user_id in t["participants"]
    }
    if not active_tours:
        await event.reply("Ты не участвуешь ни в одном активном турнире.")
        return
...
async def donate_button(event):
    await event.reply(
        ...
        f"⭐ Или купи GROM прямо тут: <b>{fmt(STAR_PACK_GROM)} GROM за {STAR_PACK_STARS} Stars</b> — оплата официальной кнопкой Telegram, без посредников.\n\n"
        "━━━━━━━━━━━━━━━\n"
        "🛒 <b>Как ещё получить GROM</b>\n\n"
        "🎁 Ежедневный бонус — 1 000 GROM раз в 24 часа\n"
        "⚔️ Дуэли — ставь и побеждай соперников\n"
        "🎰 Рулетка, Мины, Башня, Монетка, Блэкджек, Краш — играй и выигрывай\n"
        "🏆 Турниры — побеждай и забирай призовой фонд\n"
        "🏰 Клановая казна — участвуй в жизни клана\n"
        "🏢 Компании — купи компанию и владей играми в чатах\n\n"
        "💡 <i>GROM — игровая валюта бота.</i>",
        buttons=[[Button.inline(f"⭐ Купить {fmt(STAR_PACK_GROM)} GROM за {STAR_PACK_STARS} Stars", data="buy_grom_stars")]],
        parse_mode="html",
    )


@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode() == "buy_grom_stars"))
async def buy_grom_stars(event):
    payload = f"grom_pack:{STAR_PACK_STARS}:{STAR_PACK_GROM}:{event.sender_id}".encode()
    invoice = Invoice(
        currency="XTR",
        prices=[LabeledPrice(label=f"{fmt(STAR_PACK_GROM)} GROM", amount=STAR_PACK_STARS)],
        test=False,
    )
    media = InputMediaInvoice(
        title=f"{fmt(STAR_PACK_GROM)} GROM",
        description=f"Пакет {fmt(STAR_PACK_GROM)} GROM для бота GROM за {STAR_PACK_STARS} Telegram Stars.",
        invoice=invoice,
        payload=payload,
        provider="",
...
async def chats_button(event):
    ...
    await event.reply(
        ...
        f"💰 GROM в обращении: <b>{fmt(total_gram)}</b>\n"
...
async def games_button(event):
    await event.reply(
        ...
        "Команда: <code>мины 500 5</code> (ставка, кол-во мин 1-24)\n\n"
        "🏗 <b>Башня</b>\n"
        "На каждом этаже выбери одну из 3 дверей, множитель растёт с каждым этажом.\n"
...
async def company_button(event):
    ...
    for g in owned:
        if g in CHANCE_ADJUSTABLE_GAMES:
            buttons.append([Button.inline(f"🎚 Шанс: {GAME_LABELS.get(g, g)} ({get_game_chance(g)}%)", data=f"company_chance:{g}")])
...
@client.on(events.CallbackQuery(func=lambda e: e.data and e.data.decode().startswith("company_chance:")))
async def company_chance_pick(event):
    user_id = event.sender_id
    game_key = event.data.decode().split(":", 1)[1]
    cid, comp = get_company_by_owner(user_id)
    if not comp or data["game_owner"].get(game_key) != cid:
        await event.answer("Это не твоя игра.", alert=True)
        return
    if game_key not in CHANCE_ADJUSTABLE_GAMES:
        await event.answer("У этой игры нет настраиваемого шанса.", alert=True)
        return
    waiting_for_company_chance[user_id] = game_key
    await event.answer()
    await event.reply(
        f"🎚 Игра: <b>{GAME_LABELS.get(game_key, game_key)}</b>\n"
        f"Текущий шанс проигрыша игроков: <b>{get_game_chance(game_key)}%</b>\n\n"
        f"Введите число от 0 до {COMPANY_MAX_CHANCE} (компания не может ставить выше этого предела):",
        parse_mode="html",
    )


@client.on(events.NewMessage(func=lambda e: e.raw_text and waiting_for_company_chance.get(e.sender_id)))
async def company_chance_set(event):
    user_id = event.sender_id
    game_key = waiting_for_company_chance.get(user_id)
    text = event.raw_text.strip()
    ...
    cid, comp = get_company_by_owner(user_id)
    if not comp or data["game_owner"].get(game_key) != cid:
        waiting_for_company_chance.pop(user_id, None)
        return

    try:
        val = int(text)
    except ValueError:
        await event.reply(f"❗ Введите целое число от 0 до {COMPANY_MAX_CHANCE}.")
        return
    if not 0 <= val <= COMPANY_MAX_CHANCE:
        await event.reply(f"❗ Число должно быть от 0 до {COMPANY_MAX_CHANCE}.")
        return
...
# ========== МЕНЮ: ТУРНИРЫ ==========
@client.on(events.NewMessage(func=lambda e: e.raw_text == "🏆 Турниры"))
async def tournaments_button(event):
    await list_tournaments(event)
