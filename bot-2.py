...
# нажал ДРУГУЮ кнопку — старое ожидание сбрасывается, чтобы старый текст не подставился не туда.
# Искл.: "pick_rank:" — это не отмена, а продолжение уже начатого флоу назначения ранга.
# ВАЖНО: этот обработчик обязан быть зарегистрирован РАНЬШЕ всех остальных CallbackQuery-хендлеров
# (объявлен здесь, в самом начале файла), иначе он выполнится ПОСЛЕ хендлера, который только что
# установил новое ожидание ввода (например, после нажатия "Создать чек"), и тут же его сотрёт —
# из-за этого бот переставал реагировать на любой последующий текстовый ввод после нажатия кнопки.
CONTINUATION_CALLBACK_PREFIXES = ("pick_rank:",)


@client.on(events.CallbackQuery(func=lambda e: e.data and not e.data.decode().startswith(CONTINUATION_CALLBACK_PREFIXES)))
...
@client.on(events.NewMessage(func=lambda e: (
    e.sender_id in admin_flow_state
    and e.raw_text
...
async def create_promo_command(event):
    ...
    if len(parts) != 4:
        await event.reply(
            ...
            "Например: <code>/createpromo 33333 50 Обновление</code>\n\n"
            "• сумма — сколько GROM получит игрок\n"
            "• активаций — сколько раз можно использовать промокод\n"
            "• название — код промокода, одним словом, без пробелов",
            parse_mode="html",
        )
        return

    _, amount_str, uses_str, code_display = parts

    try:
        amount = int(amount_str)
        uses = int(uses_str)
    except ValueError:
        await event.reply("❗ Сумма и число активаций должны быть целыми числами.")
        return
    ...
    code_key = norm(code_display.lower())
    if code_key in data["checks"]:
        await event.reply(f"❗ Промокод «{code_display}» уже существует. Придумайте другое название.")
        return
    ...
    await event.reply(
        ...
        f"🔢 Активаций: <b>{uses}</b>\n"
        f"🔑 Код: <code>{code_display}</code>\n\n"
        f"Чтобы активировать, напишите боту:\n"
        f"<code>активировать {code_display}</code>",
        parse_mode="html",
    )
