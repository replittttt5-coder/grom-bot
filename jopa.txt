...
async def profile_button(event):
    ...
    await event.reply(
        f"👤 <b>{name}</b>\n"
        f"🆔 ID: <code>{user_id}</code>{house_line}{clan_line}{company_line}\n\n"
        f"💰 Баланс: <b>{fmt(balance)} GROM</b>\n"
...
async def company_button(event):
    ...
    if not comp:
        await event.reply(
            f"🏢 У тебя пока нет компании.\n\n"
            f"💰 Цена: <b>{fmt(get_company_price())} GROM</b>\n\n"
    ...
    await event.reply(
        f"🏢 <b>{comp['name']}</b>\n\n"
        f"💰 Баланс компании: <b>{fmt(comp['balance'])} GROM</b>\n\n"
