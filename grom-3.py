...
def emoji_tag(slot: str, fallback: str) -> str:
    """Возвращает <tg-emoji> тег, если для слота задан кастомный эмодзи, иначе обычный эмодзи."""
...
async def profile_button(event):
    ...
    company_line = f"\n🏢 Компания: <b>{comp['name']}</b>" if comp else ""
    ...
    await event.reply(
        f"{emoji_tag('profile', '👤')} <b>{name}</b>\n"
        f"{emoji_tag('id', '🆔')} ID: <code>{user_id}</code>{house_line}{clan_line}{company_line}\n\n"
        f"{emoji_tag('balance', '💰')} Баланс: <b>{fmt(balance)} GROM</b>\n"
...
async def company_button(event):
    ...
    if not comp:
        await event.reply(
            ...
            f"{emoji_tag('balance', '💰')} Цена: <b>{fmt(get_company_price())} GROM</b>\n\n"
    ...
    if comp.get("frozen"):
        ...
        frozen_line = (
            f"\n\n⛔ <b>Компания заморожена!</b> Долг: {fmt(-comp['balance'])} GROM.\n"
            f"Осталось на оплату: ~{hours_left:.1f} ч. Иначе игры будут изъяты."
        )

    await event.reply(
        f"{emoji_tag('companies', '🏢')} <b>{comp['name']}</b>\n\n"
        f"{emoji_tag('balance', '💰')} Баланс компании: <b>{fmt(comp['balance'])} GROM</b>\n\n"
        f"🎮 <b>Игры в собственности:</b>\n{games_text}\n\n"
        f"📤 <b>Открытые аукционы игр:</b>" + ("" if auctions else " нет") + frozen_line,
        buttons=buttons,
        parse_mode="html",
    )
...
@client.on(events.NewMessage(func=lambda e: e.raw_text == "🎁 Бонус"))
