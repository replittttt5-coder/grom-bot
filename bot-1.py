...
def apply_defaults(d: dict) -> dict:
    ...
    d.setdefault("gram_rate", 10)           # сколько GROM за 1 GRAM
...
async def gram_settings_start(event):
    ...
    await event.respond(
        ...
        f"Текущий курс: 1 GRAM = <b>{data['gram_rate']}</b> GROM\n"
        f"Получатель в @valyutaTG_bot: <code>{data['gram_recipient_id']}</code>\n\n"
        f"Введите новый курс (сколько GROM за 1 GRAM):",
        parse_mode="html",
    )
    await event.answer()
...
async def admin_flow_handler(event):
    ...
    if state["action"] == "gram_settings":
        ...
        if state["step"] == "recipient":
            ...
            data["gram_recipient_id"] = recipient_id
            save_data(data)
            admin_flow_state.pop(event.sender_id, None)
            await event.reply(
                f"✅ Сохранено!\nКурс: 1 GRAM = {data['gram_rate']} GROM\nПолучатель: <code>{recipient_id}</code>",
                parse_mode="html",
            )
            return
